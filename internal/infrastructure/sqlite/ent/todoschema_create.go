...
func (tsc *TodoSchemaCreate) check() error {
	if _, ok := tsc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "TodoSchema.created_at"`)}
	}
	if _, ok := tsc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "TodoSchema.updated_at"`)}
	}
...
func (u *TodoSchemaUpsertOne) UpdateNewValues() *TodoSchemaUpsertOne {
	...
	u.create.conflict = append(u.create.conflict, sql.ResolveWith(func(s *sql.UpdateSet) {
		...
		if _, exists := u.create.mutation.CreatedAt(); exists {
			s.SetIgnore(todoschema.FieldCreatedAt)
		}
	}))
	return u
}
...
func (u *TodoSchemaUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for TodoSchemaCreate.OnConflict")
	}
...
func (u *TodoSchemaUpsertOne) ID(ctx context.Context) (id uuid.UUID, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("ent: TodoSchemaUpsertOne.ID is not supported by MySQL driver. Use TodoSchemaUpsertOne.Exec instead")
	}
...
func (u *TodoSchemaUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent: OnConflict was set for builder %d. Set it on the TodoSchemaCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for TodoSchemaCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
